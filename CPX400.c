 ...
 *
 *  	v1.3 August, 2013 - Fixed QueryOutputState response 
 						  - Fixed issue where range limits where not unique to each session - Causing a problem for multiple models
						  
		v1.3.3 February 2014 - Removed 	IviDCPwrTrigger	and IviDCPwrSoftwareTrigger	from Group Capabilities		  
							 - Added support for CPX200DP and XPF60-10DP models	
							 
		v1.4.0	-  July 2014 - Fixed CPX400_ATTR_ID_QUERY_RESPONSE in simulation mode 							 
...
static IviStringValueTable CPX400_ModelTable =
{
        {CPX400DP,  	"CPX400DP"  },
        {CPX400SP, 		"CPX400SP"  },
        {CPX200DP, 		"CPX200DP"  },
		...
		{XPF6020DP,   	"XPF 60-20DP"  },
        {XPF6020SP,   	"XPF 60-20SP"  },
        {XPF6010DP,  	"XPF 60-10DP"  },
        {XPF6010SP,  	"XPF 60-10SP"  },
...
/*- Definition of structures with model specific ranges -*/

static CPX400Ranges CPX400DP_Ranges =
{
    60.0, 
    20.0,
	66.0,
...
static CPX400Ranges CPX200DP_Ranges =
{
    60.0, 
    10.0,
	66.0,
...
static ViStatus CPX400_GetRange (
    ViSession   vi,
    ViInt32     PSUModel,
    ViAttr      attributeId,
    ViReal64*   value
...
static ViStatus CPX400_QueryID(
    ViSession   vi,
    ViSession   io,
    ViInt32     bufferSize,
    ViChar      buffer[]
...
static ViStatus CPX400_GetModelFromBuffer(
    ViSession       vi,
...
static ViStatus _VI_FUNC CPX400AttrVoltageLevel_CheckCallback (ViSession vi,
                                                               ViConstString channelName,
                                                               ViAttr attributeId,
                                                               ViReal64 value);

static ViStatus _VI_FUNC CPX400AttrCurrentLimit_CheckCallback (ViSession vi,
                                                               ViConstString channelName,
                                                               ViAttr attributeId,
                                                               ViReal64 value);

static ViStatus _VI_FUNC CPX400AttrOvpLimit_CheckCallback (ViSession vi,
                                                           ViConstString channelName,
                                                           ViAttr attributeId,
                                                           ViReal64 value);

static ViStatus _VI_FUNC CPX400AttrOcpLimit_CheckCallback (ViSession vi,
                                                           ViConstString channelName,
                                                           ViAttr attributeId,
                                                           ViReal64 value);
...
ViStatus _VI_FUNC CPX400_IviInit (ViRsrc resourceName, ViBoolean IDQuery, ViBoolean reset, ViSession vi)
{
	...
	if (!Ivi_Simulating(vi))
	{
        ...
        {
            checkErr( CPX400_QueryID (vi, io, BUFFER_SIZE, id_query_buffer));
            checkErr( CPX400_GetModelFromBuffer (vi,   id_query_buffer,  &model_type));
        }		
	}
                                                 ...
                                                 ViReal64* maxCurrentLimit)
{
	...
	checkErr( Ivi_GetAttrRangeTable (vi, channelName, CPX400_ATTR_CURRENT_LIMIT, &table));   
	
	checkErr( CPX400_GetRange(vi, PSUModel, CPX400_ATTR_CURRENT_LIMIT, &value));     	
                                                 ...
                                                 ViReal64* maxVoltageLevel)
{
	...
	checkErr( Ivi_GetAttrRangeTable (vi, channelName, CPX400_ATTR_VOLTAGE_LEVEL, &table));   
	
	checkErr( CPX400_GetRange(vi, PSUModel, CPX400_ATTR_VOLTAGE_LEVEL, &value));     	
...
//		if( (outputState == CPX400_VAL_OUTPUT_CONSTANT_VOLTAGE) || (outputState == CPX400_VAL_OUTPUT_CONSTANT_CURRENT)){
//			Delay(0.2);
//			viCheckErr( viQueryf (io, "LSR%s?\n", "%hd" ,channelName, &stat_reg));  
//		}
	}
...
/*****************************************************************************
 *  Function: CPX400_QueryID
 *  Purpose:  Query the instrument for its id string.
 *****************************************************************************/
static ViStatus CPX400_QueryID(
    ViSession   vi,     /* Driver Session */
    ViSession   io,     /* IO Session */
    ViInt32     bufferSize, /* Size of buffer */
    ViChar      buffer[]    /* buffer for ID string */
)   
{
    ViStatus  error = VI_SUCCESS;
    ViUInt32  read_count;

    viCheckErr( viWrite (io, "*IDN?\n", 6, VI_NULL));
    viCheckErr( viRead (io, buffer, bufferSize-1, &read_count));
    buffer[read_count] = '\0';
...
/*****************************************************************************
 * Function: CPX400_GetModelFromBuffer
 * Purpose:  This function returns model number from buffer. If table 
 *           agesg_ModelTable does not contains this model, function
 *           returns VI_ERROR_FAIL_ID_QUERY;
 *****************************************************************************/
static ViStatus CPX400_GetModelFromBuffer(
    ViSession       vi,             /* Driver Session */
    ViConstString   buffer,         /* ID Query Buffer */
    ViInt32*        model_type      /* Model type */
)
{
    ViStatus    error = VI_SUCCESS;
    ViChar      instrument_buffer[BUFFER_SIZE];
    ViChar      model_name[BUFFER_SIZE];
	
	
	ViString	my_model = VI_NULL;

	sscanf (buffer,  "%*[^,],%256[^,]", instrument_buffer);  
		
	RemoveSurroundingWhiteSpace(instrument_buffer);
    sprintf(model_name, "%s", instrument_buffer);

	

    if (Ivi_GetValueFromTable (CPX400_ModelTable, model_name, model_type) < VI_SUCCESS)
        viCheckErr( VI_ERROR_FAIL_ID_QUERY);

	else
		checkErr(Ivi_SetAttributeViString(vi, VI_NULL,CPX400_ATTR_INSTRUMENT_MODEL,0,model_name));
...
/*****************************************************************************
 *  Function: CPX400_GetRange
 *  Purpose:  This function returns max values specified range for specified
 *            model. Valid values for attributeId are following:
...
{
    CPX400Ranges*   pranges;
 	ViStatus    error = VI_SUCCESS;  
	ViChar rdBuffer[BUFFER_SIZE];     
    /*
     * assign correct range struct for specified model
     */
    ...
    RemoveSurroundingWhiteSpace(rdBuffer);
	
	checkErr( Ivi_GetValueFromTable (CPX400_ModelTable, rdBuffer, &PSUModel) );
	...
	switch(PSUModel){
		...
		case	CPX400DP:
				pranges = &CPX400DP_Ranges;
				break;		
		case	CPX400SP:
				pranges = &CPX400DP_Ranges;
				break;
		case	CPX200DP:
				pranges = &CPX200DP_Ranges;
				break;					
		case	CPX200SP:
				pranges = &CPX200DP_Ranges;
				break;	
    ...
    switch (attributeId)
    {
    case CPX400_ATTR_CURRENT_LIMIT:
        *value = pranges->current;
        break;
    case CPX400_ATTR_VOLTAGE_LEVEL:
        *value = pranges->voltage;
        break;		
	case CPX400_ATTR_OVP_LIMIT:
		*value = pranges->ovp;
		break;
	case CPX400_ATTR_OCP_LIMIT:
		*value = pranges->ocp;
		break;		
    default:
        *value = pranges->voltage;
                                                               ...
                                                               ViReal64 value)
{
	...
	checkErr( CPX400_GetRange(vi, PSUModel, CPX400_ATTR_VOLTAGE_LEVEL, &max));
                                                               ...
                                                               ViReal64 value)
{
	...
	ViReal64			min, max; 
	IviRangeTablePtr	rangeTable = &attrCurrentLimitRangeTable;
    IviRangeTablePtr    table;
	...
	checkErr( CPX400_GetRange(vi, PSUModel, CPX400_ATTR_CURRENT_LIMIT, &max));
                                                           ...
                                                           ViReal64 value)
{
	...
	ViReal64			min, max; 
	IviRangeTablePtr	rangeTable = &attrOVPLimitRangeTable;
    IviRangeTablePtr    table;
	...
	checkErr( CPX400_GetRange(vi, PSUModel, CPX400_ATTR_OVP_LIMIT, &max));
                                                           ...
                                                           ViReal64 value)
{
	...
	ViReal64			min, max; 
	IviRangeTablePtr	rangeTable = &attrOCPLimitRangeTable;
    IviRangeTablePtr    table;
	...
	checkErr( CPX400_GetRange(vi, PSUModel, CPX400_ATTR_OCP_LIMIT, &max));
...
//	                                  "IviDCPwrMeasurement"));
	...
	checkErr (Ivi_SetAttrCheckCallbackViReal64 (vi, CPX400_ATTR_VOLTAGE_LEVEL,
	                                            CPX400AttrVoltageLevel_CheckCallback));
	...
	checkErr (Ivi_SetAttrCheckCallbackViReal64 (vi, CPX400_ATTR_OVP_LIMIT,
	                                            CPX400AttrOvpLimit_CheckCallback));
	...
	checkErr (Ivi_SetAttrCheckCallbackViReal64 (vi, CPX400_ATTR_CURRENT_LIMIT,
	                                            CPX400AttrCurrentLimit_CheckCallback));
  
	checkErr (Ivi_AddAttributeViString (vi, CPX400_ATTR_ID_QUERY_RESPONSE,
	                                    "CPX400_ATTR_ID_QUERY_RESPONSE",
	                                    "THURLBY THANDAR, CPX400DP,  0, 0.00 - 0.00",
	...
	checkErr (Ivi_SetAttrCheckCallbackViReal64 (vi, CPX400_ATTR_OCP_LIMIT,
	                                            CPX400AttrOcpLimit_CheckCallback));
